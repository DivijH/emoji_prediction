...
train_dataloader = get_dataloader(X_train, y_train, is_train=True, batch_size=16)
val_dataloader = get_dataloader(X_val, y_val, is_train=False, batch_size=64)
test_dataloader = get_dataloader(X_test, y_test, is_train=False, batch_size=64)

# Loading the BART Base model
...
for epoch_i in range(0, epochs):
    ...
    if avg_val_loss < best_val_loss:
        torch.save(model.state_dict(), f'bart_{avg_val_loss}')
...
df_stats.to_csv('Models/BART/bart.csv', encoding='utf-8')
...
plt.savefig('Models/BART/bart.png')
plt.close()

# Making predictions on the test dataset

_logger.info(f'Predicting labels for {len(test_dataloader)} test sentences...')
...
_logger.info('Completed prediction')
...
with open('Models/BART/bart_test.txt', 'w') as f:
